...
**Checkpoint**: Clarification and algorithm fixes verified — run `npm test`, confirm all tests pass, and verify the algorithm spec canonical example (1 error + 38 warnings → F, 57%) matches CLI output.
...
- **Clarification Fixes (Phase 3a)**: Retroactive corrections to completed Phase 2/3 work; T043–T053 MUST be completed before US2–US4 work begins. Within Phase 3a: T052 (loader test) FIRST — then T043 (loader impl); T044 (types) independently; then T045/T046/T047/T048 in parallel after T044; then T053 (grader signature) after T044+T047; then T049/T050 (formatter) after T053; then T051 (integration re-verify) last
