...
- [x] T054 [P] Write `tests/unit/config-loader.test.ts` BEFORE T055: (a) no `.apigrade.json` in CWD → returns empty object and no error; (b) valid `.apigrade.json` with `minGrade`, `ruleset`, `format`, `top` keys → returns typed partial `CliOptions`; (c) malformed JSON → throws descriptive error; (d) unknown keys in config are ignored — these tests MUST FAIL before T055 implementation proceeds
