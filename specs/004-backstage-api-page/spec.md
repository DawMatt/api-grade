...
1. **Given** a Backstage API page for any OpenAPI or AsyncAPI specification, **When** a user views the page, **Then** the grade letter (e.g., A, B, C, D, F), numeric percentage (e.g., 87%), and quality label (e.g., "Good", "Needs Improvement") are displayed in the Info column below the About entry.
...
1. **Given** a logged-in API owner viewing their own API's Backstage page, **When** the page loads, **Then** the full quality assessment (commentary, category-specific insights, actionable recommendations, and detailed diagnostic violations) is displayed.
2. **Given** a logged-in user who is not the API owner and is not in any additionally granted group, **When** they view the same API page, **Then** only the grade summary (letter, percentage, label) is visible — the detailed quality section is hidden.
