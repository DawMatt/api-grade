...
describe('CLI --quick-fixes-only flag', () => {
  it('--quick-fixes-only --format json matches the QuickFixOutput shape', () => {
  ...
  it('--quick-fixes-only with no --format prints human-readable text containing the filtered ruleIds', () => {
  ...
  it('--quick-fixes-only --format human also prints human-readable text with the same ruleIds', () => {
    const jsonResult = runCli([
      resolve(FIXTURES, 'openapi/poor-quality.yaml'),
      '--quick-fixes-only',
      '--format', 'json',
  ...
  it('--quick-fixes-only --min-grade still evaluates the gate against the full unfiltered result', () => {
